...
df = pd.read_csv('csv/to_shibuya/trans_min_price_2ldk_filtered.csv')
# %%
coding_list = []
for i in tqdm(df['from']):
    code = get_coordinates(i + '駅', api_key)
    coding_list.append(code)
# %%
ret_list = []
for i in tqdm(coding_list):
...
pd.concat([df, coding_df], axis=1).to_csv(
    'csv/to_shibuya/trans_min_price_2ldk_filtered_code.csv')
